...
function context_on_frame(context_ptr) {
 context_on_frame_js(context_ptr);
}

function context_on_init(context_ptr) {
 context_on_init_js(context_ptr);
}
...
function ___sigaction(sig, act, oldact) {
 if (sig == 14) {
  __sigalrm_handler = SAFE_HEAP_LOAD(act | 0, 4, 0) | 0;
  return 0;
 }
...
function emscriptenWebGLGetIndexed(target, index, data, type) {
 ...
 case 0:
  SAFE_HEAP_STORE(data | 0, ret | 0, 4);
  break;

 case 2:
  SAFE_HEAP_STORE_D(data | 0, Math.fround(ret), 4);
  break;

 case 4:
  SAFE_HEAP_STORE(data | 0, (ret ? 1 : 0) | 0, 1);
  break;
...
function _emscripten_glGetSamplerParameterfv(sampler, pname, params) {
 ...
 SAFE_HEAP_STORE_D(params | 0, Math.fround(GLctx["getSamplerParameter"](GL.samplers[sampler], pname)), 4);
}
...
function _emscripten_glGetSamplerParameteriv(sampler, pname, params) {
 ...
 SAFE_HEAP_STORE(params | 0, GLctx["getSamplerParameter"](GL.samplers[sampler], pname) | 0, 4);
}
...
function _emscripten_glGetSynciv(sync, pname, bufSize, length, values) {
 ...
 if (ret !== null) {
  SAFE_HEAP_STORE(values | 0, ret | 0, 4);
  if (length) SAFE_HEAP_STORE(length | 0, 1 | 0, 4);
 }
}
...
function _emscripten_glGetUniformIndices(program, uniformCount, uniformNames, uniformIndices) {
 ...
 program = GL.programs[program];
 var names = [];
 for (var i = 0; i < uniformCount; i++) names.push(UTF8ToString(SAFE_HEAP_LOAD(uniformNames + i * 4 | 0, 4, 0) | 0));
...
function fillMouseEventData(eventStruct, e, target) {
 ...
 if (Module["canvas"]) {
  ...
  SAFE_HEAP_STORE((idx + 13) * 4, 0, 4);
  SAFE_HEAP_STORE((idx + 14) * 4, 0, 4);
 }
 var rect = getBoundingClientRect(target);
...
function registerTouchEventCallback(target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString, targetThread) {
 ...
 var touchEventHandlerFunc = function(e) {
  ...
  for (var i in touches) {
   ...
   SAFE_HEAP_STORE((idx + 11) * 4, canvasRect ? t.clientX - canvasRect.left : 0, 4);
   SAFE_HEAP_STORE((idx + 12) * 4, canvasRect ? t.clientY - canvasRect.top : 0, 4);
   idx += 13;
   if (++numTouches > 31) {
    break;
   }
...
var asmLibraryArg = {
 ...
 "context_on_frame": context_on_frame,
 "context_on_init": context_on_init,
 "dlclose": _dlclose,
